<template>
  <common-dialog
    ...
    :data="data"
    :form-rules="rules"
    :save-func="api.saveTravelApplication"
    ...
    close-on-saved
    save-button-text="提交"
    confirm-save-text="您确定要提交该出差申请单吗？"
    :before-close="onBeforeClose"
    @close-saved="$emit('close-saved')"
    @closed="onClosed"
  >
    <template slot="form">
      <el-divider>出差申请单</el-divider>
      <el-form-item prop="suffix" label="申请标题">
        <el-input v-model="data.suffix" placeholder="标题后缀">
          <template slot="prepend">出差申请单_[员工名]_YYYYMM0001</template>
        </el-input>
      </el-form-item>
      <el-row :gutter="10">
        <el-col :span="12">
          <el-form-item label="出差开始时间" prop="beginTime">
            <el-date-picker
...
export default {
  ...
  data() {
    return {
      ...
      unit: "出差申请单",
      rules: {
        beginTime: [{ required: true, message: "请选择出差开始时间" }],
        endTime: [{ required: true, message: "请选择出差结束时间" }]
      }
    };
  },
  methods: {
