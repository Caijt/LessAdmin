<template>
  <div>
    <list
      ...
      :page="page"
      :get-page-list-func="api.getContractAnalysisList"
    >
      ...
      <template slot="column">
        <el-table-column prop="year" label="日期" width="150">
          <template slot-scope="{row}">{{row.year}}{{row.month==0?'':'-'+getMonth(row.month)}}</template>
        </el-table-column>
        ...
        <el-table-column prop="newNumber" label="合同数量" />
        <el-table-column v-if="queryParams.dimensions == 2" prop="lastNumber" label="同期合同数量">
          <template slot-scope="{row}">{{row.lastNumber == null ? "-" : row.lastNumber}}</template>
        </el-table-column>
        <el-table-column prop="newAmount" label="合同金额" />
        <el-table-column v-if="queryParams.dimensions == 2" prop="lastAmount" label="同期合同金额">
